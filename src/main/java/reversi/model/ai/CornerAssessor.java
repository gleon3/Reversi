...
/**
 * Assessment class that rates the {@link GameState} by counting the number of disks on four corners
 * of the GameField. Corner allows a player to build disks around them and provides stability to
 * the player's disks.
 *
 * <p>The best possible score can be achieved by {@link Player#WHITE} having
 * as many disks as possible on the corners, while at the same time {@link Player#BLACK} is having as
 * few disks on the corners as possible.
 */
public class CornerAssessor implements StateAssessor {
  ...
  public double computeValue(GameState state, Player minPlayer, int depth) {
    ...
    Cell[] cornerCells = {new Cell(0,0), new Cell(0, GameField.SIZE - 1),
